...
def WLSubtreeVector(
    ...
    vector = counter2vector(counter,labels_list)

    return labels_list, vector, counter
...
class WLSubtreeKernel(BaseEstimator):
    ...
    def fit(self, nodes, adj_lists, Y):
        ...
        for i, node in enumerate(nodes):
            labels_list, vector, counter = WLSubtreeVector(node,adj_lists[i],height=self.height)
            labels_lists.append(labels_list)
            vectors_lists.append(vector)
            counter_lists.append(counter)

        print(time.time() - start)

        # padding list of vectors => transform it to numpy array
        max_len_vector_lists = max(map(len, vectors_lists))
        self.max_len_vector_lists = max_len_vector_lists
        vectors_lists = list(map(lambda x: x + [0]*(max_len_vector_lists - len(x)), vectors_lists))
        ...
        X = []
        for i,counter in enumerate(counter_lists):
            vector = np.array(list(map(
                lambda x: counter2vector(counter,x,max_len_vector_lists),
                labels_lists)))
            
            x = self.kernel(vector,vectors_lists)
            X.append(x)
        X = np.array(X)
        ...
        self.vectors_lists = vectors_lists
        self.counter_lists = counter_lists


    def predict(self,node,adj):
        start = time.time()
        _, _, counter = WLSubtreeVector(node,adj,height=self.height)
        
        print("Prediction time",time.time() - start)

        vector = [
            counter2vector(counter,x,self.max_len_vector_lists) 
            for x in self.labels_lists]

        print("Prediction time",time.time() - start)
        x = self.kernel(vector,self.vectors_lists)
