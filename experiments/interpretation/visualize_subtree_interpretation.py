...
for elec_prop in ["BG","EA","IP"]:
    with open("\\".join(path) + "\\"+data_type+"\\result_"+elec_prop+".pkl","rb") as handle:
    ...
    for c,cat in enumerate(["high_bg","mid_bg","low_bg"]):
        ...
        for idx in ranks[c]:
            ...
            font = ImageFont.truetype(
                r'C:\Users\hoang\Dropbox\comp_chem_lab\MLmodel\\experiments\\interpretation\\times new roman.ttf',
                size = 50) 

            print(test_Y)
            draw.text(
                (100,430),
                "{} = {:.2f}eV".format(elec_prop,test_Y[idx]),
    ...
    side_merge_any(
        ...
        open_img = False
        ).save("\\".join(path) + "\\" + data_type+"\\final_"+elec_prop+".jpeg")
